(ns planet_labs.main
  "Kotodama WASM entrypoint for the Planet_labs clean-room actor (L4) — Clojure port.
  ...
  facts (`planet_labs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "planet_labs")
(def tier "L4")
...
(def entity-specs
  [{:entity "Field"      :plural "fields"      :id-prefix "planetla_fie"
    :fields [:name :areaHa :crop :lat :lon]   :required [:name :areaHa]
    :coerce {:areaHa :float :lat :float :lon :float} :refs {}}
   {:entity "Operation"  :plural "operations"  :id-prefix "planetla_ope"
    :fields [:fieldId :type :date :rate]      :required [:type :date]
    :coerce {:rate :float}                    :refs {:fieldId "Field"}}
   {:entity "Reading"    :plural "readings"    :id-prefix "planetla_rea"
    :fields [:fieldId :parameter :value :recordedAt] :required [:parameter :value]
    :coerce {:value :float}                   :refs {:fieldId "Field"}}
   {:entity "Machine"    :plural "machines"    :id-prefix "planetla_mac"
    :fields [:model :serial :hoursUsed]       :required [:model :serial]
    :coerce {:hoursUsed :float}               :refs {}}
   {:entity "Lot"        :plural "lots"        :id-prefix "planetla_lot"
    :fields [:productName :harvestId :quantity :traceCode] :required [:productName :quantity]
    :coerce {:quantity :float}                :refs {}}
   {:entity "Imagery"    :plural "imageries"   :id-prefix "planetla_ima"
    :fields [:fieldId :index :capturedAt :contentRef] :required [:index :capturedAt]
    :coerce {}                                :refs {:fieldId "Field"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"planet_labs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "planet_labs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :planet_labs-compat/ready)
